...
import { Search, Check, X, Filter, Trash2, Edit2, Download } from 'lucide-react';
...
export default function AdminPayments() {
  ...
  const handleSaveEdit = () => {
    ...
    if (editPayment.status !== payments.find(p => p.id === editPayment.id)?.status) {
      updatePaymentStatus(editPayment.id, editPayment.status, auth.employeeId);
    }
    setEditPayment(null);
    toast.success('Платёж обновлён');
  };

  const liveDetail = detailPayment ? payments.find(p => p.id === detailPayment.id) || detailPayment : null;
  ...
  return (
    <div className="p-4 md:p-6">
          ...
          <table className="data-table">
            <thead>
              <tr className="bg-muted/30">
                <th className="cursor-pointer" onClick={() => toggleSort('id')}>ID{sortIcon('id')}</th>
                <th className="cursor-pointer" onClick={() => toggleSort('createdAt')}>{t(lang, 'date')}{sortIcon('createdAt')}</th>
                <th>{t(lang, 'client')}</th>
                <th>{t(lang, 'account')}</th>
                <th>{t(lang, 'method')}</th>
                <th className="cursor-pointer" onClick={() => toggleSort('type')}>{t(lang, 'type')}{sortIcon('type')}</th>
                <th className="cursor-pointer" onClick={() => toggleSort('amount')}>{t(lang, 'amount')}{sortIcon('amount')}</th>
                <th className="cursor-pointer" onClick={() => toggleSort('status')}>{t(lang, 'status')}{sortIcon('status')}</th>
                <th className="w-32">{t(lang, 'actions')}</th>
              </tr>
            </thead>
            <tbody>
              {paginated.map(p => {
                    ...
                    <td onClick={e => e.stopPropagation()}>
                      <div className="flex gap-0.5">
                        {(p.status === 'New' || p.status === 'Pending') && (
                          <>
                            <button onClick={() => { updatePaymentStatus(p.id, 'Approved', auth.employeeId); toast.success('Подтверждён'); }} className="p-1.5 rounded hover:bg-muted text-emerald-600 transition-colors" title="Подтвердить"><Check size={14} /></button>
                            <button onClick={() => { updatePaymentStatus(p.id, 'Rejected', auth.employeeId); toast.success('Отклонён'); }} className="p-1.5 rounded hover:bg-muted text-destructive transition-colors" title="Отклонить"><X size={14} /></button>
                          </>
                        )}
      ...
      <Dialog open={!!liveDetail} onOpenChange={() => setDetailPayment(null)}>
        <DialogContent className="max-w-md">
          <DialogHeader><DialogTitle>Детали платежа</DialogTitle></DialogHeader>
          {liveDetail && (() => {
            const client = clients.find(c => c.id === liveDetail.clientId);
            const account = tradingAccounts.find(a => a.id === liveDetail.accountId);
            return (
              <div className="space-y-1">
                <InfoRow label="ID" value={liveDetail.id} />
                <InfoRow label="Дата создания" value={new Date(liveDetail.createdAt).toLocaleString('ru-RU')} />
                ...
                <InfoRow label="Тип" value={typeLabel(liveDetail.type)} />
                <InfoRow label="Сумма" value={`$${liveDetail.amount.toLocaleString()} ${liveDetail.currency}`} />
                <InfoRow label="Метод оплаты" value={liveDetail.paymentMethod} />
                <InfoRow label="Статус" value={liveDetail.status} />
                {liveDetail.processedAt && <InfoRow label="Обработан" value={new Date(liveDetail.processedAt).toLocaleString('ru-RU')} />}

                {(liveDetail.type === 'Withdrawal' || liveDetail.wallet) && (
                  <div className="mt-4 pt-3 border-t">
                    <h4 className="text-sm font-semibold mb-2">Реквизиты вывода</h4>
                    <InfoRow label="Кошелёк / Реквизиты" value={liveDetail.wallet} />
                  </div>
                )}
                ...
                {liveDetail.comment && (
                  <div className="mt-4 pt-3 border-t">
                    ...
                    <p className="text-sm text-muted-foreground">{liveDetail.comment}</p>
                  </div>
                )}

                <div className="flex flex-wrap gap-2 pt-4 border-t mt-4">
                  {(liveDetail.status === 'New' || liveDetail.status === 'Pending') && (
                    <>
                      <Button size="sm" className="bg-emerald-600 hover:bg-emerald-700 text-white" onClick={() => { updatePaymentStatus(liveDetail.id, 'Approved', auth.employeeId); toast.success('Подтверждён'); }}>
                        <Check size={14} className="mr-1" /> Подтвердить
                      </Button>
                      <Button size="sm" variant="destructive" onClick={() => { updatePaymentStatus(liveDetail.id, 'Rejected', auth.employeeId); toast.success('Отклонён'); }}>
                        <X size={14} className="mr-1" /> Отклонить
                      </Button>
                    </>
                  )}
                  <Button size="sm" variant="outline" onClick={() => { setDetailPayment(null); setEditPayment({ ...liveDetail }); }}>
                    <Edit2 size={14} className="mr-1" /> Редактировать
                  </Button>
                  <Button size="sm" variant="outline" className="text-destructive hover:text-destructive ml-auto" onClick={() => confirmDelete('Удаление платежа', 'Удалить этот платёж?', () => { deletePayment(liveDetail.id); setDetailPayment(null); toast.success('Удалён'); })}>
