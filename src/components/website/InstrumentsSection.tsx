    ...
    <section className="py-24 md:py-32 bg-background">
      <div className="container mx-auto px-6">
        <AnimatedSection className="text-center mb-16">
          <span className="text-primary text-sm font-semibold uppercase tracking-wider">{w("instrTag")}</span>
          <h2 className="text-3xl md:text-5xl font-bold text-foreground mt-3 mb-5">{w("instrTitle")}</h2>
            ...
            <AnimatedSection key={item.nameKey} delay={i * 0.1}>
              <div className="group relative p-6 rounded-2xl border border-border bg-card hover:border-transparent hover:shadow-2xl transition-all duration-500 overflow-hidden h-full">
                ...
                <div className="relative z-10">
                  ...
                  <h3 className="text-lg font-bold text-foreground mb-1">{w(item.nameKey)}</h3>
                  <p className="text-primary text-sm font-semibold mb-3">{w(item.countKey)}</p>
